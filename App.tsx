...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (isAutoPlay) {
      autoPlayTimerRef.current = window.setInterval(nextSlide, 8000);
    } else {
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-[#1a1a1a] text-white selection:bg-yellow-500/30">
      ...
      <header className="fixed top-0 left-0 w-full p-6 md:p-8 flex justify-between items-start z-50 pointer-events-none">
        <div className="flex items-center gap-4 md:gap-6 pointer-events-auto bg-black/60 backdrop-blur-xl p-3 md:p-4 px-5 md:px-6 rounded-2xl border border-white/10 shadow-2xl">
        ...
        <div className="flex gap-3 pointer-events-auto">
          <button 
            ...
            className={`p-4 rounded-2xl border transition-all flex items-center gap-2 backdrop-blur-md ${isAutoPlay ? 'border-yellow-500 text-yellow-500 bg-yellow-500/10' : 'border-white/10 text-white/60 bg-black/40'}`}
          >
            <span className="text-[10px] font-bold tracking-widest uppercase">{isAutoPlay ? "Dừng Tự Động" : "Tự Động"}</span>
          </button>
          <button 
            onClick={() => setShowToc(!showToc)}
            className="bg-black/60 backdrop-blur-xl p-4 rounded-2xl border border-white/10 text-yellow-500 hover:bg-yellow-600 hover:text-black transition-all group"
          >
      ...
      <footer className="fixed bottom-0 left-0 w-full p-6 md:p-10 flex justify-between items-end z-50 pointer-events-none">
        <div className="bg-black/60 backdrop-blur-xl p-4 px-6 border border-white/10 rounded-2xl pointer-events-auto shadow-2xl">
        ...
        <div className="flex gap-3 pointer-events-auto">
          <button 
            onClick={prevSlide}
            className="w-14 h-14 flex items-center justify-center border border-white/10 rounded-2xl bg-black/40 hover:bg-yellow-600 hover:text-black transition-all backdrop-blur-xl shadow-xl"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {showToc && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-6 md:p-24 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-4 pr-4 custom-scrollbar">
              {SLIDES.map((slide, idx) => (
                <button
                  ...
                  className={`group text-left p-4 rounded-xl transition-all border-l-4 ${currentIndex === idx ? 'bg-yellow-600/20 border-yellow-500 shadow-inner' : 'border-white/5 hover:border-yellow-500/50 hover:bg-white/5'}`}
                >
